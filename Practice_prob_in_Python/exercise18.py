    ...
    def remove_duplicates(self):
        ...
        while current is not None:
            if current.value not in my_set:
                ...
                #The actual linked list structure is determined by the next pointers of the nodes.
                #So, we can simply change the next pointer of the previous node to skip the current node
                prev.next=current.next
                self.length-=1
            current=current.next
        
    """
Why current Doesn't Affect the Final List:

1. The `current` pointer is only used for traversal.
   - It helps inspect each node but doesn't directly modify the list.

2. The list structure is entirely determined by the `next` pointers of the nodes.

3. When `prev.next` is updated to "skip" a node:
   - The skipped node is effectively removed from the list because no other node points to it.

4. Even though `current` continues to iterate:
   - The changes to `prev.next` are already applied to the linked list in memory.

In summary:
- `current` is simply a traversal tool, 
- While `prev` and `prev.next` are what actually modify the linked list structure.
"""
